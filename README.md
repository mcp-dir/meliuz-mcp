...
### Méliuz no Claude, ChatGPT e agentes de IA via Open Finance

Conecte sua conta do **Méliuz** ao Claude, ChatGPT e agentes de IA via Open Finance Brasil. Pergunte sobre saldos, extratos, faturas de cartão e investimentos do Méliuz em linguagem natural. Regulado pelo Banco Central, somente leitura.

- 🏦 **Méliuz via Open Finance Brasil**, regulado pelo Banco Central
...
**Manual** (se o deeplink não abrir): [claude.ai/customize/connectors](https://claude.ai/customize/connectors) → **+** → **Adicionar conector personalizado** → cole **Nome** `Méliuz MCP` e **URL** `https://api.mcp.ai/p_meliuz`.

### Cursor

[➕ Instalar Méliuz MCP no Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=meliuz&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9tZWxpdXoifQ==)

### VS Code (Copilot Chat)

[➕ Instalar Méliuz MCP no VS Code](vscode:mcp/install?name=meliuz&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_meliuz%22%7D)
...
```
Quanto gastei no cartão do Méliuz esse mês?
Resuma minha fatura do Méliuz em aberto
Qual meu saldo na conta do Méliuz?
Como está minha carteira de investimentos no Méliuz?
```
...
**Como funciona?**
Via Open Finance Brasil (regulado pelo Banco Central). Você conecta sua conta Méliuz com consentimento explícito; a IA lê saldos, extratos, faturas e investimentos. Somente leitura.

**A IA pode movimentar dinheiro?**
Não. 100% leitura — Open Finance não permite movimentação por essa via.

**Só funciona com o Méliuz?**
Este MCP é focado no Méliuz. Para conectar vários bancos de uma vez, use o Banco MCP (api.mcp.ai/banco).
...
- 📧 [meliuz@mcp.ai](mailto:meliuz@mcp.ai)
- 🐛 [GitHub Issues](https://github.com/mcp-dir/meliuz-mcp/issues)
- 📄 [docs/](docs/)
...
MIT — veja [LICENSE](LICENSE). O servidor MCP em `api.mcp.ai/p_meliuz` é proprietário (hosted); este repositório (manifestos, docs, skills) é MIT.
